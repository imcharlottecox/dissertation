...
export function drawEdges(context: MCRenderContext) {
    ...
    const edges = g.select<SVGGElement>("g.edges")
        ...
        .attr("stroke-width", d => weightedThickness ? Math.min((Math.max(0.05, d.probability * 5)), 3) : 1)
        .attr("opacity", d => {
            if( focusClickedNodeId){ return focusClickEdgeIds.has(d.id) ? 1 : 0.06;} 
            if (filterActiveEdges){ return filterActiveEdges.has(`${d.from}->${d.to}`) ? 1 : 0.06;}   
            return 1;})
        ...
        .join(
            enter => {
                ...
                n.filter(d => d.kind === "base" && acceptingStates.includes(d.id))
                    .append("circle")
                    .attr("r", d =>nodeRadius-3) //double ring
                    .attr("fill", "none")
