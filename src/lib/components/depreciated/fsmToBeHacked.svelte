<script lang="ts">
    ...
    import { computeLevelsMap, computeNodePositions, splitWarp, computeNodePositionsWithBackbone } from "../fsm/fsmHierarchicalHelpers";
    ...
    function updateDepthDisplay() {
        ...
        for (const id of active) {
            ...
            if (sg?.depthLevel && sg.depthLevel > maxDepth) {
                maxDepth = sg.depthLevel;
            }
        }
    ...
    onMount(() => {
        ...
        const maxL = Math.max(...levels.values());
        for (const acc of acceptingStates) {
        if (levels.has(acc)) levels.set(acc, maxL + 1);
        }
        // const pos = computeNodePositions(levels, graphWidth, graphHeight, padding);

        const pos = computeNodePositionsWithBackbone(
            levels,
            graphWidth,
            graphHeight,
            padding,
