...
if (char === ")") {
    ...
    const returnState = stack[stack.length - 1]; // "Wait"
    const newStack = stack.slice(0, -1);
    
    // manually fire Wait → After_atom (the ) transition from Wait)
    const afterWait = transitions.filter(
        t => t.from === returnState && labelMatchesChar(t.label, char)
    );
    for (const t of afterWait) {
        const moved: Config = { state: t.to, stack: newStack };
        const closed = epsilonClose(moved, warpMap, callMap, returnStates);
        for (const c of closed) {
            if (!next.has(configKey(c))) next.set(configKey(c), c);
        }
    }
    // if no transition from Wait on ), the ) is unmatched for this config — don't add anything
    continue;
}
