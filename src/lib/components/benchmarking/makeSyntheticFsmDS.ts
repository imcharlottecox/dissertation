...
export function makeSyntheticFsmDS(context: {n: number; seed: number; includeBackbone?: boolean; noSelfLoops: boolean; noDupeEdges: boolean; linearDensity?: number}){
    const {n, seed = 22, includeBackbone = true, noSelfLoops = true, noDupeEdges = true, linearDensity=3} = context;
    ...
    const targetEdges = Math.floor(0.05 * macPossible); //sqrt case
    // const targetEdges = Math.min(Math.floor(linearDensity*n), macPossible);
