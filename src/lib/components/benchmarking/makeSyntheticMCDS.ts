...
export function makeSyntheticMCDS(context: {n: number; seed: number; noSelfLoops?: boolean; linearDensity?: number}){
    const {n, seed = 22, noSelfLoops = true, linearDensity = 3} = context;

    const rand = createRandomSeed(seed);
    const markovStates = Array.from({length: n}, (_,i) => `S${i}`);
    const maxPossible = noSelfLoops ? n*(n-1) : n*n;
    const targetEdges = Math.floor(0.05 * maxPossible);
    // const targetEdges = Math.min(Math.floor(linearDensity*n), maxPossible);
